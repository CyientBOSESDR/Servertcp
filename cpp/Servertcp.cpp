...
PREPARE_LOGGING(Servertcp_i)

Servertcp_i::Servertcp_i(const char *uuid, const char *label) :
    Servertcp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

Servertcp_i::~Servertcp_i()
{
}

void Servertcp_i::constructor()
{
    /***********************************************************************************
     This is the RH constructor. All properties are properly initialized before this function is called 
    ***********************************************************************************/
}
void Servertcp_i::start() throw (CF::Resource::StartError, CORBA::SystemException)
{
	Servertcp();
}

int Servertcp_i::Servertcp(void)
{
    ...
    printf("%s\n",buffer);
    std::cout<<"Recieved buffer msg : " << buffer;
   // LOG_INFO(Server_i, buffer);
    send(new_sock,hello,strlen(hello),0);
    printf("\nmessage sent to server\n");
    std::cout<<"Msg sent in buffer : " << hello;
    }
    return 0;
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void Servertcp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &Servertcp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &Servertcp_i::<callback method>)
        in the constructor.
        ...
        //Add to Servertcp.cpp
        Servertcp_i::Servertcp_i(const char *uuid, const char *label) :
            Servertcp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &Servertcp_i::scaleChanged);
            addPropertyListener(status, this, &Servertcp_i::statusChanged);
        }

        void Servertcp_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void Servertcp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
...
************************************************************************************************/
int Servertcp_i::serviceFunction()
{
